...
#		-C $(TARGET_DIR)/ . | gzip -9n > $(BIN_DIR)/$(IMG_PREFIX)$(if $(PROFILE_SANITIZED),-$(PROFILE_SANITIZED))-rootfs.tar.gz
	...
	cp -rf $(STAGING_DIR_ROOT)/* $(TARGET_DIR)/
	rm -rf $(LINUX_DIR)/tmp
	mkdir -p $(LINUX_DIR)/tmp/
	pwd
	@echo ARCH=$(ARCH)
	@echo CROSS_COMPILE=$(CROSS_COMPILE)
	make ARCH=$(ARCH) -C $(LINUX_DIR) modules_install INSTALL_MOD_PATH=tmp
	cp -rf `find $(LINUX_DIR)/tmp/ -name *.ko` $(TARGET_DIR)/lib/modules/4.1.15/
#	打包模块
	$(TAR) --numeric-owner --owner=0 --group=0 --sort=name \
		-jcf $(BIN_DIR)/modules.tar.bz2 -C $(LINUX_DIR)/tmp/lib/modules/ ./
	#打包根文件系统
	$(TAR) --numeric-owner --owner=0 --group=0 --sort=name \
		-jcf $(BIN_DIR)/rootfs.tar.bz2 -C $(TARGET_DIR) ./
	#拷贝image文件
	cp -f $(LINUX_DIR)/arch/arm/boot/zImage $(BIN_DIR)/
	#拷贝dts文件
	cp -f $(LINUX_DIR)/arch/arm/boot/dts/imx6ull-alientek-emmc.dtb $(BIN_DIR)/
  endef
endif
